/*
 * Simple ESP32 UDP Audio Playback Test - FIXED VERSION
 * Receives audio data via UDP and plays it through MAX98357A amplifier
 * 
...
// UDP Configuration
WiFiUDP udp;
const int udpPort = 1235;  // Port to listen for audio data
...
void setup() {
  ...
  Serial.println("\n===================================");
  Serial.println("ESP32 UDP Audio Playback Test");
  Serial.println("FIXED VERSION - v1.1");
  Serial.println("===================================\n");
  ...
  Serial.println("Send WAV file via UDP to port 1235");
...
void loop() {
  ...
  if (packetSize) {
    ...
    if (len > 0) {
      ...
      if (len % 2 != 0) {
        len--;  // Drop the last byte if odd
      }
      
      // Play the audio data immediately
      size_t written = 0;
      esp_err_t err = i2s_write(I2S_NUM_0, audioBuffer, len, &written, 100 / portTICK_RATE_MS); // 100ms timeout

...
class IntegratedSpeechAnalysisApp:
    ...
    def test_esp32_speaker(self):
        """Send a test command to ESP32 to verify speaker works via UDP"""
        if self.prompt_socket:
            try:
                print(f"\n[TEST] Sending speaker test command to {self.esp32_ip}:{self.prompt_port}...")
                self.prompt_socket.sendto(b"PLAYTEST", (self.esp32_ip, self.prompt_port))
                print("[TEST] Command sent! You should hear a beep from the ESP32 speaker.")
                time.sleep(1)
            except Exception as e:
                print(f"[TEST] Failed to send test command: {e}")
    
    def test_playback_pipeline(self):
        """Generate a test tone and send it to ESP32 to verify full playback pipeline"""
        import numpy as np
        
        print("\n[TEST] Generating 2-second test tone (1kHz)...")
        # Generate a 1kHz sine wave for 2 seconds
        sample_rate = 44100
        duration = 2
        t = np.linspace(0, duration, int(sample_rate * duration), False)
        tone = np.sin(2 * np.pi * 1000 * t) * 16000  # 1kHz at 50% volume
        tone = tone.astype(np.int16)
        
        print(f"[TEST] Generated {len(tone)} samples ({len(tone) * 2} bytes)")
        
        # For test tone, we already have 16-bit data, so wrap it in a way that _stream_audio_to_esp32
        # will handle correctly (it expects 32-bit bytes, so we convert to 32-bit first)
        tone_32bit = tone.astype(np.int32) << 16  # Convert 16-bit to 32-bit format
        tone_bytes = tone_32bit.tobytes()
        print(f"[TEST] Converted to 32-bit format: {len(tone_bytes)} bytes")
        
        # Send IMPROVE message first
        if self.prompt_socket:
            self.prompt_socket.sendto(b"IMPROVE:Test tone coming!", (self.esp32_ip, self.prompt_port))
            time.sleep(0.5)
            
            # Stream the test tone
            print("[TEST] Streaming test tone to ESP32...")
            self._stream_audio_to_esp32(tone_bytes, sample_rate=sample_rate)
    ...
    async def run_esp32_analysis(self, duration=10):
        ...
        try:
            ...
            # Save the recording with analysis results FIRST (before playback)
            # This ensures recording is saved even if user interrupts during playback
            ...
            if improvement_tips:
                ...
                # Also playback the recorded audio through ESP32 speaker
                self.send_improvement_to_esp32(improvement_tips, self.esp32_audio_buffer)
            ...
            if "timeout" in str(e).lower():
                ...
                try:
                    ...
                    if improvement_tips:
                        # Send improvement suggestions to ESP32 after showing scores
                        # Also playback the recorded audio through ESP32 speaker
                        self.send_improvement_to_esp32(improvement_tips, reduced_buffer)
    ...
    def send_improvement_to_esp32(self, improvement_tips, audio_buffer=None):
        """
        Send improvement suggestions to the ESP32 to display on its OLED
        and optionally playback the recorded audio through the speaker
        :param improvement_tips: Improvement suggestions to display
        :param audio_buffer: Optional audio data to playback (bytes or numpy array)
        """
        if self.prompt_socket:
            try:
                ...
                # Debug audio buffer info
                if audio_buffer is None:
                    print("[Playback] No audio_buffer provided (None)")
                elif len(audio_buffer) == 0:
                    print("[Playback] audio_buffer is empty (0 bytes)")
                else:
                    print(f"[Playback] Audio buffer provided: {len(audio_buffer)} bytes")
                    print(f"[Playback] Starting audio stream to ESP32 speaker...")
                    self._stream_audio_to_esp32(audio_buffer)
            except Exception as e:
                print(f"Failed to send improvement tips to ESP32: {e}")
    
    def _stream_audio_to_esp32(self, audio_data, sample_rate=44100, chunk_size=1024):
        """
        Stream audio data to ESP32 for playback through speaker
        :param audio_data: Audio data as bytes or numpy array
        :param sample_rate: Sample rate of audio
        :param chunk_size: Size of each UDP packet
        """
        try:
            import numpy as np
            import time
            ...
            if audio_data is None:
                print("[Playback] ERROR: audio_data is None!")
                return
            
            audio_len = len(audio_data) if hasattr(audio_data, '__len__') else 'unknown'
            print(f"[Playback] Received audio_data type={type(audio_data)}, len={audio_len}")
            
            # Convert audio data to numpy array
            # IMPORTANT: ESP32 sends 32-bit audio, we need to convert to 16-bit
            if isinstance(audio_data, bytes):
                # First interpret as 32-bit (which is what ESP32 sends)
                audio_32bit = np.frombuffer(audio_data, dtype=np.int32)
                print(f"[Playback] Raw 32-bit samples: {len(audio_32bit)}")
                
                # DEBUG: Show first few raw samples
                print(f"[Playback] First 5 raw 32-bit values: {audio_32bit[:5]}")
                print(f"[Playback] Min: {audio_32bit.min()}, Max: {audio_32bit.max()}")
                
                # Convert to 16-bit (MAX98357A expects 16-bit)
                # For I2S 32-bit audio, the actual sample is usually in upper bits
                # Try different conversion methods based on data range
                max_val = np.abs(audio_32bit).max()
                if max_val > 0:
                    # Normalize to 16-bit range
                    audio_array = (audio_32bit / max_val * 32767).astype(np.int16)
                    print(f"[Playback] Normalized to 16-bit (max_val={max_val})")
                else:
                    # If all zeros, try alternative: shift by 8 for 24-bit data
                    audio_array = (audio_32bit >> 8).astype(np.int16)
                    print(f"[Playback] Shifted by 8 (24-bit assumption)")
                
                print(f"[Playback] First 5 converted values: {audio_array[:5]}")
                print(f"[Playback] Converted to 16-bit: {len(audio_array)} samples")
            else:
                audio_array = np.array(audio_data, dtype=np.int16)
                print(f"[Playback] Using provided array: {len(audio_array)} samples")
            
            # Ensure mono (average stereo channels if needed)
            ...
            print(f"[Playback] Streaming {total_samples} samples ({len(audio_array.tobytes())} bytes) to {self.esp32_ip}:{self.prompt_port}")
            print(f"[Playback] Target ESP32 IP: {self.esp32_ip}, Port: {self.prompt_port}")
            ...
            for i in range(0, total_samples, chunk_size):
                ...
                if packet_count <= 3:
                    print(f"[Playback] Packet #{packet_count}: {len(chunk_bytes)} bytes, first sample={chunk[0] if len(chunk) > 0 else 'N/A'}")
                
                # Send chunk to ESP32
                try:
                    bytes_sent = self.prompt_socket.sendto(chunk_bytes, (self.esp32_ip, self.prompt_port))
                    if bytes_sent != len(chunk_bytes):
                        print(f"[Playback WARNING] Only sent {bytes_sent}/{len(chunk_bytes)} bytes!")
                except Exception as send_err:
                    print(f"[Playback ERROR] Failed to send packet #{packet_count}: {send_err}")
                    break
                
                samples_sent += len(chunk)
                
                # Small delay to prevent overwhelming ESP32
                time.sleep(chunk_size / sample_rate * 0.8)  # Slightly faster than real-time
                
                # Progress update every 100 packets
                if packet_count % 100 == 0:
                    progress = (samples_sent / total_samples) * 100
                    print(f"[Playback] Progress: {progress:.1f}% ({packet_count} packets sent)")
            
            print(f"[Playback] Sent {packet_count} packets total, waiting for playback...")
            
            # Wait for playback to complete (audio duration + buffer)
            audio_duration = total_samples / sample_rate
            wait_time = audio_duration + 1.0  # Audio duration + 1 second buffer
            print(f"[Playback] Audio duration: {audio_duration:.1f}s, waiting {wait_time:.1f}s...")
            time.sleep(wait_time)
            
            # Send end signal
            self.prompt_socket.sendto(b"PLAYBACK_END", (self.esp32_ip, self.prompt_port))
            
            elapsed = time.time() - start_time
            print(f"[Playback] Complete! Sent {samples_sent}/{total_samples} samples in {elapsed:.1f}s")
            
        except Exception as e:
            print(f"[Playback] Error streaming audio: {e}")
            # Still try to send end signal
            try:
                self.prompt_socket.sendto(b"PLAYBACK_END", (self.esp32_ip, self.prompt_port))
            except:
                pass

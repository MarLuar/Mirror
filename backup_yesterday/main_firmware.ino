/*
 * Mirror Speech Analyzer - ESP32 Firmware
 * Version: 1.02
 * 
 * Changelog:
 * v1.02 - Added delay(1) to allow WiFi stack to process incoming UDP packets
 * v1.01 - Added debug logging for audio playback
 * v1.00 - Initial version with INMP441 mic, MAX98357A speaker, OLED display
 */
...
// Audio Playback Buffer (for receiving recorded audio from laptop)
#define PLAYBACK_BUFFER_SIZE 1024
int16_t playbackBuffer[PLAYBACK_BUFFER_SIZE];
bool isPlayingBack = false;
unsigned long playbackStartTime = 0;
unsigned long lastPacketTime = 0;
#define PLAYBACK_TIMEOUT 5000  // Stop playback if no data for 5 seconds

// UDP for receiving playback audio (use same port as prompt but different socket or reuse)
// We'll use the existing promptUdp but check for audio packets
...
// Initialize MAX98357A Amplifier (I2S1)
void initAmplifier() {
  i2s_config_t i2s_config = {
  ...
  if (err != ESP_OK) {
    Serial.println("ERROR: Failed to install I2S1 for amplifier!");
    return;
  }
  ...
  Serial.println("Amplifier (MAX98357A) initialized on I2S1");
  ...
  delay(100);
  playBeep(1000, 200);  // Should hear this on boot!
  Serial.println("Speaker test complete (did you hear a beep?)");
}

// Play a short beep on the speaker
void playBeep(int frequency, int duration_ms) {
  ...
  for (int i = 0; i < samples; i += 64) {
    ...
    for (int j = 0; j < chunk; j++) {
      float t = (float)(i + j) / 44100;
      buffer[j] = (int16_t)(sin(2 * PI * frequency * t) * 16000);
    }
    size_t written;
    i2s_write(I2S_NUM_1, buffer, chunk * sizeof(int16_t), &written, portMAX_DELAY);
  }
}
...
// Start audio playback mode (triggered when IMPROVE message received)
void startPlaybackMode() {
  isPlayingBack = true;
  playbackStartTime = millis();
  lastPacketTime = millis();
  Serial.println("\n=== Audio Playback Started ===");
  Serial.println("Waiting for audio packets on port 1235...");
  
  // Show playback indicator on OLED
  display.stopscroll();
  display.fillRect(0, 56, 128, 8, SSD1306_BLACK);  // Clear bottom line
  display.setCursor(0, 56);
  display.print("[Playing speech...]");
  display.display();
}
...
// Handle incoming playback audio data
void handlePlaybackAudio() {
  if (!isPlayingBack) return;
  
  // Check for incoming audio packets on prompt port (laptop sends audio here)
  int packetSize = promptUdp.parsePacket();
  if (packetSize > 0) {
    lastPacketTime = millis();
    
    // Debug: log first few packets (reset counter when playback starts)
    static int totalPackets = 0;
    static bool wasPlayingBack = false;
    if (!wasPlayingBack && isPlayingBack) {
      totalPackets = 0;  // Reset counter when playback starts
    }
    wasPlayingBack = isPlayingBack;
    totalPackets++;
    if (totalPackets <= 10) {
      Serial.printf("[DEBUG] Received packet #%d, size=%d bytes\n", totalPackets, packetSize);
    }
    
    // Read audio data (expecting 16-bit PCM)
    int len = promptUdp.read((uint8_t*)playbackBuffer, min(packetSize, PLAYBACK_BUFFER_SIZE * 2));
    
    if (len > 0) {
      // Play the audio through I2S amplifier
      size_t bytes_written;
      i2s_write(I2S_NUM_1, playbackBuffer, len, &bytes_written, portMAX_DELAY);
      
      // Debug output every 50 packets
      if (totalPackets % 50 == 0) {
        Serial.printf("Playback: received %d packets total\n", totalPackets);
      }
    }
  }
  
  // Check for playback timeout (no data for 5 seconds)
  if (millis() - lastPacketTime > PLAYBACK_TIMEOUT) {
    Serial.println("Playback timeout - stopping");
    stopPlaybackMode();
  }
}
...
void setup() {
  ...
  Serial.println("  BEEP   - Test speaker");
  Serial.println("===================================");
}
...
void handleSerialCommands() {
  if (Serial.available()) {
    ...
    else if (command.equalsIgnoreCase("BEEP") || command.equalsIgnoreCase("TEST")) {
...
void loop() {
  ...
  // Handle audio playback if in playback mode
  if (isPlayingBack) {
    handlePlaybackAudio();
  }
  ...
  if (promptSize) {
    ...
    Serial.printf("[UDP] Received %d bytes: '%s'\n", promptSize, currentPrompt);
    
    // Check if this is a special command
    if (strcmp(currentPrompt, "PLAYTEST") == 0) {
      // Test speaker via UDP
      Serial.println("[TEST] Playing test tone via UDP command!");
      playBeep(1000, 500);
      display.clearDisplay();
      display.setCursor(0, 0);
      display.println("Speaker Test!");
      display.display();
    }
    else if (strcmp(currentPrompt, "PROCESSING_AUDIO") == 0) {
    ...
    // Check if this is a playback end signal
    else if (strcmp(currentPrompt, "PLAYBACK_END") == 0) {
      stopPlaybackMode();
      // Clear the playback indicator
    ...
    // Check if this is a "HOW_TO_IMPROVE" message
      ...
      // Start audio playback of the recorded speech
      startPlaybackMode();
    }
    else {
  ...
  // Small delay to allow WiFi stack to process incoming packets
  // This is critical for receiving UDP prompts while streaming audio
  delay(1);
}

...
void record_wav_streaming() {
  const uint32_t max_record_time = 5;  // Reduced to 5 seconds to prevent memory issues
  ...
  uint8_t* buffer = (uint8_t*)malloc(512); // Smaller buffer to save memory
  if (!buffer) {
  ...
  while (digitalRead(BUTTON_PIN) == LOW && (millis() - startTime < max_record_time * 1000)) {
    size_t bytes_read = 0;
    esp_err_t ret = i2s_read(I2S_NUM_0, buffer, 512, &bytes_read, pdMS_TO_TICKS(100));
...
String send_to_elevenlabs_stt(String filename) {
  ...
  if (file_size > 150000) { // Further reduced to 150KB to prevent memory issues
    Serial.println("File too large for STT request (>150KB)");
    file.close();
    return "";
  }
  ...
  // Check if we have enough free heap to allocate the request
  size_t free_heap = heap_caps_get_free_size(MALLOC_CAP_8BIT);
  if (free_heap < total_size + 10000) { // Leave 10KB margin
    Serial.println("Not enough memory to process this file");
    file.close();
    http.end();
    return "";
  }
  ...
  // Copy the start boundary
  memcpy(complete_body, body_start.c_str(), body_start.length());

  // Read the file content directly into the complete body
  size_t bytes_read = file.read(complete_body + body_start.length(), file_size);

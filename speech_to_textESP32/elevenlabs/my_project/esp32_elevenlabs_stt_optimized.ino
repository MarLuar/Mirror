...
void record_wav_streaming() {
  const uint32_t max_record_time = 15;  // Reduced to 15 seconds to save memory
  ...
  uint8_t* buffer = (uint8_t*)malloc(1024); // Increased buffer size for efficiency
  if (!buffer) {
...
String send_to_elevenlabs_stt_streaming(String filename) {
  ...
  if (file_size > 500000) { // Limit to 500KB
    Serial.println("File too large for STT request (>500KB)");
    file.close();
    return "";
  }

  // Load only a small portion of the file to check if it has content
  uint8_t header_check[100];
  file.seek(0);
  size_t header_bytes_read = file.read(header_check, 100);
  ...
  HTTPClient http;
  http.begin(elevenlabs_stt_url); // Start with the API URL
  ...
  // Use HTTPClient's beginPost to start the request
  http.beginPost("");
  ...
  http.write((uint8_t*)body_start.c_str(), body_start.length());
  ...
  if (chunk_buffer) {
    ...
    while (bytes_sent < file_size) {
      ...
      http.write(chunk_buffer, bytes_read);
      bytes_sent += bytes_read;
    }
    free(chunk_buffer);
  }

  // Send the ending part of the request
  http.write((uint8_t*)body_end.c_str(), body_end.length());

  // Finish the request and get response
  httpResponseCode = http.endPost();
  ...
  if (httpResponseCode == 200) {
    ...
    if (deserializeJson(doc, response) == DeserializationError::Ok) {
      if (doc.containsKey("text") && !doc["text"].as<String>().isEmpty()) {
        ...
        Serial.println("API returned empty transcription");
      }
    } else {

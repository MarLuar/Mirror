...
// Current 3 prompts displayed
char displayPrompts[3][22];  // Shortened for display (21 chars + null)
int selectedPromptIndex = 0; // Which of the 3 is selected (0, 1, or 2)
...
void show3Prompts() {
  ...
  // Randomly select 1 of the 3 to be the active prompt
  selectedPromptIndex = random(3);  // 0, 1, or 2
  
  // Copy to display prompts (truncated for display)
  for (int i = 0; i < 3; i++) {
    strncpy(displayPrompts[i], promptPool[indices[i]], 21);
    displayPrompts[i][21] = '\0';
  }
  ...
  display.println("Speak this prompt:");
  display.println("");
  
  // Show all 3 with ">>" marking the selected one
  for (int i = 0; i < 3; i++) {
    if (i == selectedPromptIndex) {
      display.print(">>");  // Arrow marks selected
    } else {
      display.print("  ");  // Spaces for others
    }
    display.print(i + 1);
    display.print(".");
    display.println(displayPrompts[i]);
  }
